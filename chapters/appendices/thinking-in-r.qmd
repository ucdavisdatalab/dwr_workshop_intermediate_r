...
This appendix contains materials from _Thinking in R_, a retired workshop
series about how R works and how to examine code critically. The major topics
of this chapter are how R stores and locates variables (including functions)
defined in your code and in packages, and how some of R's object-oriented
...
### What's an Environment? {#sec-whats-an-environment}
...
Except for `names`, the functions introduced in @sec-whats-an-environment
default to the local environment if you don't set the `envir` parameter. This
makes them convenient for inspecting or modifying the local environment's
frame:
...
### Call Environments {#sec-call-environments}
...
On the other hand, in the `get_tea` function from @sec-lexical-scoping, `tea`
...
called the **empty environment** which will be explained in
@sec-the-search-path.
...
### The Search Path {#sec-the-search-path}
...
discussed in @sec-call-environments, functions with side effects make code
...
the oldest and most widely-used. This section focuses on S3, while
@sec-other-object-systems provides an overview of R's other OOP systems.
...
## Other Object Systems {#sec-other-object-systems}
