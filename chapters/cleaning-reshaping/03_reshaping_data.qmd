...
## Tidy Datasets {#sec-tidy-datasets}
...
datasets. In fact, the [Tidyverse][tidyverse] packages (see @sec-the-tidyverse)
are designed from the ground up for working with tidy datasets. Tidy datesets
have also been adopted as a standard in other software, including various
...
described in @sec-splitting-strings:
...
needs to be pivoted longer (@sec-columns-into-rows). The result of the pivot
will be easier to understand if you rename the columns as their years first.
...
Our calculation is to subtract one from the fiscal year if the month is seven
or less so we have to convert the months to numbers from text abbreviations.
Let's use our old friend the `fast_strptime` function from the `lubridate`
package (@sec-the-lubridate-package).
